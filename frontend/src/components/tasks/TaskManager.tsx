          ...
          {TASK_STATUS_VALUES.map((status) => {
            ...
            return <div key={status} className={`min-w-0 rounded-xl border p-3 ${STATUS_STYLES[status]}`}>
              ...
              <div className="max-h-[calc(100vh-15rem)] min-h-24 space-y-3 overflow-y-auto pr-1">
                {statusTasks.map((task) => <TaskCard key={task.id} task={task} projectId={projectId} onStatusChange={handleStatusChange} onEdit={handleEdit} onDelete={handleDelete} isStatusUpdating={updateStatus.isPending} isDeleting={deleteTask.isPending} />)}
              </div>
            </div>
          })}
        </div>
